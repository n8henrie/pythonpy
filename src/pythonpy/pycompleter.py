...
def parse_string(input):
    if current_raw(input).startswith("--"):
        ...
        if "import".startswith(current_raw(input)):
            options = ["'import"]
        elif current_raw(input).startswith("import ") or current_raw(
            input
        ).startswith("from "):
            module_completion, module_list = get_completerlib()
            ...
            if options:
                options = [
                    x.rstrip(" ")
                    for x in options
                    if x.startswith(current_prefix(input))
                ]
        else:
            options = complete_all(
                current_prefix(input), defaultdict(lambda: None)
            )
...
def main():
    ...
    if len(input) == 0:
        ...
        if len(current_list(input)) > 1 and max(
            list(map(len, options))
        ) + 1 >= len(current_raw(input)):
